...
with DAG(
    ...
    producer_1_task = BashOperator(
        ...
        bash_command=f"cd {PROJECT_ROOT}/kafka && python3 producer.py --id 1",
        retries=2,
        max_active_tis_per_dag=1
    )

    producer_2_task = BashOperator(task_id="producer_2_task",
        bash_command=f"cd {PROJECT_ROOT}/kafka && python3 producer.py --id 2",
        retries=2,
        max_active_tis_per_dag=1
    )
    
    producer_3_task = BashOperator(task_id="producer_3_task",
        bash_command=f"cd {PROJECT_ROOT}/kafka && python3 producer.py --id 3",
        retries=2,
        max_active_tis_per_dag=1
    )
